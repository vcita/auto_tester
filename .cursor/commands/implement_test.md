...
7. **Matter entity agnosticism**: Do not hardcode matter entity labels (e.g. "Properties", "Clients", "Add property", "Delete properties?"). Use regex or positional selectors so tests work across verticals. For "Add &lt;entity&gt;" use **`tests._params.ADD_MATTER_TEXT_REGEX`** (entity list in **`tests/_params/matter_entities.yaml`**). See `.cursor/rules/project.mdc` § Matter Entity Name Agnosticism.
