# Services Category – Rules Validation Report

**Date**: 2026-01-30  
**Scope**: All tests under `tests/scheduling/services/` (services category; no _setup/_teardown in this category)
...
**Summary**: 10 rule areas; 8 Pass, 0 Fail, 2 N/A.
...
|-----------|--------|-------|
| 1. Navigation (no reload/goto internal) | ✅ Pass | No `page.reload()`, `page.goto()`, or `page.refresh()` in code; only comments reference past healing |
| 2. Outcome verification | ✅ Pass | State-changing steps have verification and Expected Result describes observable outcomes |
| 3. Text input (press_sequentially) | ✅ Pass | All `.fill()` documented (number spinbutton or clear) in test.py and script.md |
...
| 6. Wait strategy (no arbitrary waits) | ✅ Pass | All `wait_for_timeout` ≤500 ms with allowed/brief comments; no long fixed waits in code or script |
| 7. Context / prerequisites consistency | N/A | Context chain documented; no cross-category chain in scope |
| 8. Matter entity name agnosticism | ✅ Pass | No hardcoded matter-entity-only locators in services tests |
| 9. No retries for actions | ✅ Pass | Scroll-to-find loops only; "retry" only in changelog/script narrative, not user-action retries |
| 10. Test cleanup and teardown | ✅ Pass | No _setup/_teardown; create→delete pairs; context cleared in delete_service and delete_group_event |
...
- **Result**: ✅ **Pass**
- **Details**: Matches only in comments (e.g. “HEALED … Replaced page.reload() with UI navigation”). No active reload/goto/refresh in test code.

---

## 2. Outcome verification (state-changing steps)

- **Rule**: For any state-changing step (create, update, delete, etc.), the test must verify the outcome (e.g. item in list, values, removal). Expected Result must state what to assert.
...
- **Result**: ✅ **Pass**
- **Details**: All `.fill()` uses have comments: “fill is OK for number spinbutton” or “Clear existing value” in both test.py and script.md (create_service, edit_service, edit_group_event, create_group_event).
...
- **Result**: ✅ **Pass**
- **Details**: All six scripts have multiple VERIFIED PLAYWRIGHT CODE blocks with “How verified” and “Wait for” per step (63 blocks across the 6 scripts).
...
- **Rule**: No arbitrary time waits. Use event-based waits with long timeout. `wait_for_timeout()` only for small allowed delays (e.g. 100–500 ms with “allowed”/“brief” comment).
- **Source**: `.cursor/rules/phase3_code.mdc`, `.cursor/rules/phase2_script.mdc`
- **Check**: Grep for `wait_for_timeout` in test.py and script.md; flag calls >500 ms or without allowed comment.
- **Result**: ✅ **Pass**
- **Details**: All `wait_for_timeout` in test.py and script.md are ≤500 ms (100, 200, 300, 500) with “Brief delay”, “Brief settle (allowed)”, or similar. No 1000 ms or 3000 ms in current script.md or test.py (changelog has historical entries only).
...
- **Source**: `.cursor/rules/phase1_steps.mdc`
- **Check**: For tests that pass context (create→edit→delete for service and group event), steps.md and docstrings match.
- **Result**: **N/A**
- **Details**: Context chain is documented (created_service_name, created_group_event_name, etc.). No cross-category context in scope. Docstrings and steps.md aligned.

---

## 8. Matter entity name agnosticism

- **Rule**: Do not hardcode a single matter-entity label (e.g. “Properties”, “Clients”) in locators or assertions.
- **Source**: `.cursor/rules/project.mdc`
- **Check**: Grep for hardcoded entity-only strings in services tests.
- **Result**: ✅ **Pass**
- **Details**: No matches for “Properties”, “Delete properties?”, “Add property”, “1 SELECTED OF … PROPERTIES”, or similar. Services tests use “Services”, “My Services”, “New service”, etc., which are not matter-entity labels under this rule.
...
- **Check**: Grep for retry/retries/for attempt in range in test.py and script.md; exclude scroll-to-find.
- **Result**: ✅ **Pass**
- **Details**: The only `for … in range(max_scrolls)` loops are scroll-to-find (load list until item visible or end). “Retry” appears only in create_service/changelog (historical) and create_service/script.md “How verified” text (narrative), not as user-action retry loops in test.py.

---

## 10. Test cleanup and teardown

- **Rule**: After category execution, system should be clean. Objects created in tests deleted in later tests; context cleared after deletion.
- **Source**: `.cursor/rules/project.mdc`
