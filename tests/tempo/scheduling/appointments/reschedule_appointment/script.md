...
appointment.wait_for(state='visible', timeout=5000)
```
...
**VERIFIED PLAYWRIGHT CODE**:
```python
page.wait_for_url("**/app/appointments/**", timeout=5000)
...
new_time_option = outer_iframe.locator("li.k-item:visible").filter(has_text="9:30am").first
new_time_option.wait_for(state='visible', timeout=5000)
```
...
| Option | Pros | Cons |
|--------|------|------|
| `outer_iframe.locator("li.k-item:visible").filter(has_text="9:30am").first` | Avoids duplicate hidden Kendo options and differs from create flow's 10:00am appointment | Depends on rendered Kendo option class |
| `outer_iframe.get_by_role('option', name='9:30am')` | Semantic | Can match duplicate hidden options |

**CHOSEN**: visible `li.k-item` filtered by `9:30am` - Keeps the final time-changed assertion meaningful after the create flow selects 10:00am and avoids duplicate hidden options.

**VERIFIED PLAYWRIGHT CODE**:
```python
# The dropdown contains visible option elements with time text like "9:30am", "10:00am", etc.
...
back_btn.click()
page.wait_for_url("**/app/calendar**", timeout=5000)
```
