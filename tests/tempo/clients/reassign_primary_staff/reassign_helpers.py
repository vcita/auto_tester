...
#
# Budget is 180s: in isolation the email arrives well under 90s, but during a full
# back-to-back suite run the automation mail pipeline is under load and delivery was
# observed at ~100s. 180s keeps the eventual-consistency headroom without masking a
# genuine "email never sent" regression (which still fails after the budget).
EMAIL_POLL_BUDGET_SECONDS = 180
