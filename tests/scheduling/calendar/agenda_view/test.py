...
def test_agenda_view(page: Page, context: dict) -> None:
    ...
    create_appointment_via_api(context, "service1", client, "next_day_0", "15:30", "Auto Staff1")
    create_appointment_via_api(context, "service2", client, "next_day_0", "07:00", "Auto Staff2")
    create_event_via_api(context, "event1", "next_day_0", "07:00", "Auto Staff2")
    ...
    assert_calendar_items(
        ...
        "Agenda",
        [
            {"item_type": "appointment", "state": "scheduled", "item_title": f"{context['created_client_name']}, {context['calendar_services']['service1']['name']}"},
            {"item_type": "event", "state": "scheduled", "attendance": f"0/2 {context['calendar_services']['event1']['name']}"},
        ],
    )
