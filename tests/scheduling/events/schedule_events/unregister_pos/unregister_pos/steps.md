# Unregister from BO and CP, then pay via POS

Migrated from `automation-js/features/tempo/scheduling-events.feature` scenario 2B
("create event and pays with POS", VCITA2-14026).

**Precondition (setup):** isolated account with `point_of_sale` enabled (default), a `$100`
...
4. **Pay `nir karpin`'s `$100` through Point of Sale** (attendee → payment status → take
   payment → POS checkout → record Cash).
