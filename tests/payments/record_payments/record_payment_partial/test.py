...
def test_record_payment_partial(page: Page, context: dict) -> None:
    ...
    partial_amount = "20"
    _fill_payment_amount(dialog, amount_input, partial_amount)
    _commit_payment_amount(dialog, amount_input)

    _select_cash_method(invoice_scope, dialog)

    _assert_recordable_amount(dialog, amount_input, partial_amount)
    record_button = dialog.get_by_role("button", name="Record")
    record_button.wait_for(state="visible", timeout=5000)
    record_button.click()

    dialog.wait_for(state="hidden", timeout=5000)
    context["recorded_payment_partial_amount"] = partial_amount
