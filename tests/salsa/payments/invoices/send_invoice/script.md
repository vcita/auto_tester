# Record Invoice Payment - Detailed Script

> **Status**: Verified with headed run and 10-iteration headless stress test
> **Last Updated**: 2026-05-18
> **Generated By**: Cursor AI stabilization workflow

## Initial State
- User is logged in.
- Payment gateway is NOT connected.

## Actions

### Step 1: Open Billing & Invoicing
- **Action**: Navigate via Sales sidebar -> Billing & Invoicing.
**VERIFIED PLAYWRIGHT CODE**:
```python
page.get_by_role("button", name="Sales").click()
page.get_by_text("Billing & Invoicing", exact=True).click()
```

### Step 2: Open invoice
- **Action**: Open first invoice.
**VERIFIED PLAYWRIGHT CODE**:
```python
invoice_scope.get_by_role("link", name=re.compile("INVOICE #")).first.click()
```

### Step 3: Record payment
- **Action**: Click "Take payment", choose "Record payment", and record the default invoice balance as cash.
**VERIFIED PLAYWRIGHT CODE**:
```python
invoice_scope.get_by_role("button", name=re.compile(r"^Take payment")).click()
invoice_scope.get_by_role("menuitem", name=re.compile("Record payment")).click()
```

## Success Verification
- Record payment dialog closes successfully.
- Save `recorded_invoice_payment_status` in context.

