"""Apply a cart percentage coupon in the client-portal checkout.

Migrates automation-js features/salsa/coupons-pay.feature
(scenario: Client apply cart percentage coupon in cp checkout).
"""
...
EXPECTED_AMOUNT = "Amount received: $108.00"


def test_cart_percentage(page: Page, context: dict) -> None:
    ...
    print("  Step 2: Create 10% entire-cart coupon via API")
    coupon_code = create_coupon_via_api(
        context, "Coupon entire cart - percentage", "percent", "10"
    )
    ...
    try:
        ...
        print("  Step 4: Verify success page shows 'Payment confirmed' and $108.00")
        assert_payment_success(
            cp_page, title=CONFIRMED_TITLE, subtitle=CONFIRMED_SUBTITLE, amount=EXPECTED_AMOUNT
        )
        print("  [OK] cart percentage coupon -> $108.00")
    finally:
        cp_context.close()
