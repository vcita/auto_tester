"""Setup for the "paying for product via Point of Sale" scenario.

POS is enabled (account default), so `take_payment` opens Point of Sale. Mirrors
the products.feature Background plus the scenario's "assigns a product to client
via API" Given.
"""
...
def setup_pay_pos(page: Page, context: dict) -> None:
    username = context.get("username")
    password = context.get("password")
    if not (username and password):
        raise ValueError("Isolated account username and password are missing from context")

    print("  Setup Step 1: Log in to isolated account (POS enabled)")
