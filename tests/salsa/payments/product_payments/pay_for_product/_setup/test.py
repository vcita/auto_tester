"""Setup for the "paying for product" scenario (isolated account).

Mirrors the products.feature Background plus the scenario's "denies feature flags:
point_of_sale" so `take_payment` opens the legacy record-payment dialog, and the
"assigns a product to client via API" Given.
"""
...
def setup_pay_for_product(page: Page, context: dict) -> None:
