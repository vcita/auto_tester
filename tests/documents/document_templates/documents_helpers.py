...
def grab_document_link(page: Page, file_name: str) -> str:
    ...
    kebab.wait_for(state="visible", timeout=UI_TIMEOUT)
    kebab.click()
    ...
    copy_option.wait_for(state="visible", timeout=UI_TIMEOUT)
    copy_option.click()
...
def _read_link_from_dialog(page: Page) -> str:
    ...
    link_text.wait_for(state="visible", timeout=UI_TIMEOUT)
