...
- `select_first_client()` → first `[data-qa="CrmTable-All"] tbody tr` rendered checkbox
  wrapper (identity-agnostic; count is what's asserted) → assert `1 SELECTED OF 12 CLIENTS`.
...
- `search_clients("first")` → `[data-qa="CrmTable-All-actionBar-searchBar"]` → wait the
  settled unselected summary `11 CLIENTS` → `select_first_client()` → assert `1 SELECTED OF 11 CLIENTS`.
...
- `assert_summary_text(expected)` reads `[data-qa="summary-text"]` via inner_text
  (rendered, respects text-transform like the legacy Selenium getText) and compares
  case-insensitively after whitespace-normalizing; polls ≤5s for the re-render.
...
- All UI waits ≤5s; no fixed sleeps; table readiness via skeleton-gone wait.
