...
- **Action**: Click
- **Target**: "Quick Actions" button in the left sidebar
- **Element hints**:
  - `get_by_role("button", name="Quick Actions")`
  - Button with "+" icon and text "Quick Actions"
  - Located in left sidebar, near top
- **Wait for**: Quick Actions modal/menu to appear

### Step 2: Click "Add property" in Quick Actions Menu
- **Action**: Click
- **Target**: "Add property" option in the Quick Actions menu
- **Element hints**:
  - Look for menu element with text "Add property" under PROPERTIES section
  - `locator('#client').click()` (discovered via Playwright MCP)
- **Wait for**: Property creation form/dialog to appear (inside iframe)

### Step 3: Click "Show more" to expand contact fields
- **Action**: Click
...
- The Quick Actions menu has multiple sections: SCHEDULE, PROPERTIES, SALES, OTHER
